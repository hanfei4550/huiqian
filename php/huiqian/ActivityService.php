...
class ActivityService
{
    function getCurrentActivity()
    {
        ...
        $stmt = $conn->prepare("SELECT user_id,activity_id,banner,is_validate_user FROM t_usercenter_user_activity ua INNER JOIN t_activitycenter_activity a ON ua.activity_id=a.id WHERE a.begin_time > ? AND a.end_time <= ?");
        $stmt->bind_param('ss', $beginTime, $endTime);
        $stmt->execute();
        $stmt->bind_result($user_id, $activity_id, $banner, $is_validate_user);
        while ($stmt->fetch()) {
